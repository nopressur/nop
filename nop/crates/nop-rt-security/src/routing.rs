...
use nop_rt_templates::error::{self, ErrorRenderer};
...
fn parse_id_route(raw_path: &str) -> Option<nop_content_store::flat_storage::ContentId> {
    let trimmed = raw_path.trim();
    if trimmed.is_empty() {
        return None;
    }
...
mod tests {
    ...
    struct CacheHarness {
        _fixture: nop_testing::test_fixtures::TestFixtureRoot,
        cache: PageMetaCache,
    }
