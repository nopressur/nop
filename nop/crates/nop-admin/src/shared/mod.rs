...
use log;
use nop_config::{PasswordHashingParams, ValidatedConfig, ValidatedUsersConfig};
use nop_rt_headers::{generate_csp_nonce, set_strict_csp};
use nop_rt_iam::AuthRequest;
use nop_rt_templates::AdminSpaShellContext;
use nop_rt_templates::RequestTools;
use nop_rt_templates::render_minijinja_template;
use serde::Serialize;
...
pub async fn render_admin_spa_shell_response(
    ...
    let html = render_admin_spa_shell_html(config, request_tools, bootstrap, &csp_nonce).await?;
    Ok(HttpResponse::Ok()
