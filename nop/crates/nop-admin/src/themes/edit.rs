...
use nop_rt_bootstrap::paths::RED_THEME_THEME;
use nop_rt_headers::{generate_csp_nonce, set_strict_csp};
...
fn build_theme_path(theme_name: &str, themes_dir: &Path) -> std::result::Result<PathBuf, String> {
    nop_library::validate_new_file_name(theme_name)?;
    let filename = format!("{}.theme", theme_name);
    validate_new_file_path(&filename, themes_dir)
}
...
async fn render_theme_error(
    ...
    let html =
        shared::render_admin_spa_shell_html(config, request_tools, Some(bootstrap), &csp_nonce)
            .await?;
...
pub async fn themes_new(
    ...
    let default_theme_path = runtime_paths.themes_dir.join("default.theme");
...
pub async fn themes_create(
    ...
    let theme_path = match build_theme_path(&payload.name, &runtime_paths.themes_dir) {
        Ok(path) => path,
        Err(error_msg) => {
...
pub async fn themes_customize(
    ...
    let theme_path = match build_theme_path(&theme_name, &runtime_paths.themes_dir) {
        Ok(path) => path,
        Err(error_msg) => {
    ...
    let canonical_path = match security::canonical_path_checks(
        &theme_path,
        &runtime_paths.themes_dir.to_string_lossy(),
