...
mod tests {
    ...
    fn test_validate_new_file_name() {
        ...
        assert!(validate_new_file_name("Invalid Name").is_err());
        assert!(validate_new_file_name("invalid.name").is_err());
        assert!(validate_new_file_name("invalid@name").is_err());
        assert!(validate_new_file_name("invalid/name").is_err());
        assert!(validate_new_file_name("invalid\\name").is_err());
        assert!(validate_new_file_name(&"a".repeat(129)).is_err());
    }

    #[test]
    fn test_directory_creation_restrictions() {
        assert!(validate_new_file_name("dir1/dir2").is_err());
        assert!(validate_new_file_name("dir1\\dir2").is_err());
    ...
    fn test_validate_and_sanitize_user_name() {
        ...
        assert!(validate_and_sanitize_user_name("   ").is_err());
        assert!(validate_and_sanitize_user_name("A").is_err());
        assert!(validate_and_sanitize_user_name(&"A".repeat(257)).is_err());
