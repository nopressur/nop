...
fn main() {
    ...
    let repo_root = manifest_dir
        ...
        .to_path_buf();
    let nop_root = repo_root.join("nop");
    let scripts_root = repo_root.join("scripts");
    let builtin_root = nop_root.join("builtin");
    let admin_root = nop_root.join("ts").join("admin");
    let admin_output = builtin_root.join("admin");
    let login_root = nop_root.join("ts").join("login");
    let site_root = nop_root.join("ts").join("site");
    ...
    ensure_bulma_css(&bulma_script, &bulma_version, &bulma_css, &nop_root);
    ensure_ace_assets(
        &ace_script,
        &ace_version,
        &builtin_root,
...
fn ensure_bulma_css(
    ...
    println!("Bulma CSS missing; running updater");
...
fn ensure_ace_assets(
    ...
    println!("Ace assets missing; running updater");
...
fn ensure_admin_spa(admin_root: &Path, admin_output: &Path) {
    ...
    if needs_admin_build(admin_root, &outputs) {
        println!("Building admin SPA via npm run build");
...
fn ensure_site_bundle(site_root: &Path, site_output: &Path) {
    if needs_site_build(site_root, site_output) {
        println!("Building site bundle via npm run build");
...
fn ensure_login_spa(login_root: &Path, builtin_root: &Path, version_file: &Path) -> String {
    ...
    if needs_build {
        ...
        println!("Building login SPA via npm run build");
