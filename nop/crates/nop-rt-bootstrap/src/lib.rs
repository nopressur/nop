...
use nop_config::{Config, ConfigError, ValidatedConfig};
...
mod tests {
    use super::*;
    use nop_config::{ServerProtocol, ServerRole};
    ...
    fn new_fixture(prefix: &str) -> TempDir {
    ...
    fn bootstrap_creates_defaults_when_missing() {
        let fixture = new_fixture("bootstrap-default");
        ...
        match password {
            ...
            PasswordRecord::LegacyHash(hash) => {
                panic!("expected provider password record, got legacy hash {hash}")
            }
        }
    ...
    fn bootstrap_is_idempotent() {
        let fixture = new_fixture("bootstrap-idempotent");
    ...
    fn bootstrap_skips_users_for_oidc() {
        let fixture = new_fixture("bootstrap-oidc");
        let config_path = fixture.path().join("config.yaml");
        let config = r#"server:
  host: "127.0.0.1"
  port: 8080
