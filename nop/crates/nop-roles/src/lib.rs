...
pub use nop_management_contract::AccessRule;
            ...
            if !record.roles.is_empty() {
                ...
                for role in &record.roles {
                    match normalize_role(role) {
                        ...
                        Err(err) => {
                            warn!(
                                "Skipping invalid role '{}' in tag '{}' ({}): {}",
                                role, tag, record.name, err
                            );
                        }
                    }
                }
...
pub fn has_access_for_roles(
    resolved_roles: &ResolvedRoleSet,
    user_roles: Option<&[String]>,
) -> bool {
    match resolved_roles {
        ResolvedRoleSet::Public => true,
        ResolvedRoleSet::Deny => {
            user_roles.is_some_and(|roles| roles.iter().any(|role| role == ADMIN_ROLE))
        }
        ResolvedRoleSet::Restricted(required_roles) => {
            let Some(user_roles) = user_roles else {
                return false;
            };

            if user_roles.iter().any(|role| role == ADMIN_ROLE) {
                return true;
            }
...
mod tests {
    ...
    fn resolve_roles_honors_union_and_intersect() {
        ...
        let union = resolve_roles_for_tags(&["union".to_string()], &tag_map);
        assert_eq!(
            union,
        ...
        let intersect =
            resolve_roles_for_tags(&["union".to_string(), "intersect".to_string()], &tag_map);
    ...
    fn resolve_roles_invalid_entries_result_in_deny() {
        ...
        let resolved = resolve_roles_for_tags(&["invalid".to_string()], &tag_map);
        assert_eq!(resolved, ResolvedRoleSet::Deny);
    }

    #[test]
    fn has_access_for_roles_allows_admin() {
        let user_roles = vec![ADMIN_ROLE.to_string()];
        let resolved = ResolvedRoleSet::Restricted(vec!["viewer".to_string()]);
        assert!(has_access_for_roles(&resolved, Some(user_roles.as_slice())));
    }
