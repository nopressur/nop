    ...
    pub fn register_prefix(&self, prefix: impl Into<String>, handler: WellKnownHandler) {
        ...
        match self.prefixes.write() {
            Ok(mut guard) => {
                ...
                guard.sort_by_key(|entry| Reverse(entry.0.len()));
            }
            Err(_) => {
...
async fn serve_well_known(
    ...
    if relative_path.is_empty() {
        return serve_404_with_app_name(&config.app.name);
    }
    ...
    serve_404_with_app_name(&config.app.name)
}
...
fn serve_404_with_app_name(app_name: &str) -> Result<HttpResponse> {
    let html = fallback_404_html(app_name);
    ...
    fn build_test_config() -> ValidatedConfig {
        ValidatedConfig {
            ...
            tls: Some(nop_config::TlsConfig {
                mode: nop_config::TlsMode::SelfSigned,
