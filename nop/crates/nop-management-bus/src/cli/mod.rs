...
pub async fn run_cli(runtime_root: &Path, tokens: Vec<String>) -> i32 {
    ...
    match crate::cli_helper::execute(runtime_root, command).await {
...
mod tests {
    ...
    fn resolves_unambiguous_prefix() {
        ...
        let command = registry.resolve_command(&["al".into(), "fir".into()]);
    ...
    fn detects_ambiguous_command_prefix() {
        ...
        let err = registry
            .resolve_command(&["alpha".into(), "li".into()])
            .unwrap_err();
    ...
    fn detects_unknown_domain() {
        let registry = build_test_registry();
        let err = registry
            .resolve_command(&["unknown".into(), "first".into()])
            .unwrap_err();
