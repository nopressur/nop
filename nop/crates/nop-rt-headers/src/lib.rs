...
fn is_public_content<B>(
    ...
    match cache.resolved_roles(&alias) {
        Some(nop_rt_page_cache::ResolvedRoles::Public) => true,
        Some(nop_rt_page_cache::ResolvedRoles::Restricted(_)) => false,
        Some(nop_rt_page_cache::ResolvedRoles::Deny) => false,
        None => false,
    }
}
...
fn determine_alias(path: &str) -> String {
    ...
    nop_content_store::flat_storage::canonicalize_alias(path).unwrap_or_default()
}
...
mod tests {
    ...
    impl TestFixtureRoot {
        ...
        fn runtime_paths(&self) -> std::io::Result<RuntimePaths> {
