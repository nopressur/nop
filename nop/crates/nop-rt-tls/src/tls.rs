...
use log::warn;
use nop_config::{AcmeChallenge, AcmeEnvironment, TlsConfig, TlsMode, ValidatedConfig};
...
const RELOAD_INTERVAL: StdDuration = StdDuration::from_secs(5);
...
impl FileReloadingResolver {
    fn new(cert_path: std::path::PathBuf, key_path: std::path::PathBuf) -> io::Result<Self> {
        Self::new_with_interval(cert_path, key_path, RELOAD_INTERVAL)
    }
    ...
    fn new_with_interval(
        ...
        let certified_key = Arc::new(ArcSwap::from(certified_key));
...
fn spawn_reload_thread(
    ...
    std::thread::spawn(move || {
        loop {
            ...
            let latest = match latest_modified(&cert_path, &key_path) {
                Ok(latest) => latest,
                Err(err) => {
                    warn!("TLS reload check failed: {}", err);
                    continue;
                }
            };
            ...
            match load_certified_key(&cert_path, &key_path) {
                Ok(updated) => {
                    certified_key.store(updated);
                    last_modified = latest;
                }
                Err(err) => {
                    warn!("TLS reload failed: {}", err);
                }
            }
        }
    });
...
mod tests {
    ...
    fn build_test_config(
        servers: Vec<nop_config::ServerListenerConfig>,
        tls: TlsConfig,
    ) -> ValidatedConfig {
        ValidatedConfig {
            servers,
    ...
    fn https_main_and_well_known() -> Vec<nop_config::ServerListenerConfig> {
        vec![
            nop_config::ServerListenerConfig {
                name: Some("main-https".to_string()),
                role: nop_config::ServerRole::Main,
                host: "127.0.0.1".to_string(),
                port: 8443,
                protocol: nop_config::ServerProtocol::Https,
            },
            nop_config::ServerListenerConfig {
                name: Some("well-known".to_string()),
                role: nop_config::ServerRole::WellKnown,
                host: "127.0.0.1".to_string(),
                port: 8080,
                protocol: nop_config::ServerProtocol::Http,
            },
        ]
    }
    ...
    fn file_reloading_resolver_updates_cert() {
        ...
        let resolver = FileReloadingResolver::new_with_interval(
            cert_path.clone(),
            key_path.clone(),
        ...
        let updated = updated.expect("expected cert to reload");
    ...
    fn self_signed_regenerates_on_state_mismatch() {
        ...
        assert!(cert_covers_domains(&cert_path, &["new.example.com".to_string()]).unwrap());
    }
