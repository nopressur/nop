...
use nop_config::{AuthMethod, PasswordHashingParams, ValidatedConfig, ValidatedUsersConfig};
use nop_iam_passwords::{
    PasswordError, PasswordProviderBlock, build_password_provider_block, generate_salt_hex,
    verify_front_end_hash,
};
use nop_library::validate_email_field;
use std::fmt;
use std::sync::Arc;
...
fn normalize_email(email: &str) -> Result<String, UserServiceError> {
    let normalized = email.trim().to_lowercase();
    validate_email_field(&normalized).map_err(UserServiceError::Validation)?;
    Ok(normalized)
}
...
impl UserServices {
    ...
    pub fn current_front_end_salt(&self, email: &str) -> UserServiceResult<String> {
        let normalized = normalize_email(email)?;
        let front_end_len = self.password_params.front_end.salt_len;
        match self.get_user(&normalized)? {
            Some(user) => match user.password.as_ref() {
                Some(block) => Ok(block.front_end_salt.clone()),
                None => generate_salt_hex(front_end_len).map_err(UserServiceError::Password),
            },
            None => generate_salt_hex(front_end_len).map_err(UserServiceError::Password),
        }
    }

    pub async fn issue_password_change_salt(
        &self,
        email: &str,
        ttl: Duration,
    ) -> UserServiceResult<PasswordSaltIssue> {
        let normalized = normalize_email(email)?;
        let current_front_end_salt = self.current_front_end_salt(&normalized)?;
        let next_front_end_salt = generate_salt_hex(self.password_params.front_end.salt_len)
            .map_err(UserServiceError::Password)?;
        let (change_token, _token) = self
            .password_change_store
            .issue(&normalized, next_front_end_salt.clone(), ttl)
            .await
            .map_err(|err| UserServiceError::Iam(err.message().to_string()))?;
        Ok(PasswordSaltIssue {
            change_token,
    ...
    pub async fn validate_password_change_token(
        &self,
        email: &str,
        change_token: &str,
        next_front_end_salt: &str,
    ) -> UserServiceResult<()> {
        let normalized = normalize_email(email)?;
        let token = self
            .password_change_store
        ...
        let Some(token) = token else {
            return Err(UserServiceError::Validation(
                "Password change token expired or invalid".to_string(),
            ));
        };
        if token.email != normalized {
            return Err(UserServiceError::Validation(
                "Password change token does not match user".to_string(),
            ));
        }
        if token.next_front_end_salt != next_front_end_salt {
            return Err(UserServiceError::Validation(
                "Password change token does not match requested salt".to_string(),
            ));
        }
        Ok(())
    }

    pub async fn invalidate_password_change_token(
        &self,
        change_token: &str,
    ) -> UserServiceResult<()> {
        self.password_change_store
...
mod tests {
    ...
    use nop_iam_passwords::{
        PasswordProviderBlock, build_password_provider_block, derive_front_end_hash,
    };
    ...
    #[tokio::test]
    async fn issue_password_change_salt_returns_existing_front_end_salt() {
        let block = build_password_block();
        let user = User {
            email: "user@example.com".to_string(),
            name: "User".to_string(),
        ...
        let issued = services
            .issue_password_change_salt("user@example.com", Duration::from_secs(30))
            .await
            .expect("password salt");

        assert_eq!(issued.current_front_end_salt, block.front_end_salt);
        assert_eq!(issued.expires_in_seconds, 30);
        assert!(!issued.change_token.is_empty());
        assert!(!issued.next_front_end_salt.is_empty());
    }

    #[tokio::test]
    async fn validate_password_change_token_rejects_wrong_user() {
        let user = User {
            email: "user@example.com".to_string(),
            name: "User".to_string(),
            password: Some(build_password_block()),
