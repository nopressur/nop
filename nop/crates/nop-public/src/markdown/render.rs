...
use nop_rt_templates::{load_template, render_template};
...
<body><div>{content}</div></body></html>"#
    ...
    let escaped_title = crate::nav::html_escape(title);
    let escaped_app_name = crate::nav::html_escape(&render_ctx.config.app.name);
