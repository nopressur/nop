...
use crate::cache::{HtmlCacheEnvelope, finalize_html_response};
use crate::nav::generate_navigation_with_user;
use crate::{PageRenderContext, PublicRequestContext};
use actix_web::{HttpResponse, Result};
use nop_content_store::flat_storage::{blob_path, content_id_hex};
...
pub async fn serve_markdown_alias(
    ...
    let sanitizer = &ctx.render_tools.html_sanitizer;
    let error_renderer = &ctx.request_tools.error_renderer;
    let template_engine = ctx.request_tools.templates.as_ref();
    let req = ctx.req;
    ...
    let (file_exists, has_access) =
        check_file_access(cache, alias, user_roles, dev_mode_bypass_allowed);
    ...
    let object = match cache.get_by_alias(alias) {
        Some(object) => object,
        None => return nop_rt_templates::error::serve_404(error_renderer, Some(template_engine)),
    };
    ...
    let canonical_file_path = match security::canonical_path_checks(
        &blob_path,
        &ctx.runtime_paths.content_dir.to_string_lossy(),
    ...
    let content = match fs::read_to_string(&canonical_file_path).await {
        Ok(content) => content,
        Err(_) => return nop_rt_templates::error::serve_500(error_renderer, Some(template_engine)),
    };
    ...
    let rendered_html = match generate_html(&RenderRequest {
        ...
        Err(error) => {
            ...
            return nop_rt_templates::error::serve_500(error_renderer, Some(template_engine));
        }
    };

    let navigation =
        generate_navigation_with_user(cache, user, config.navigation.max_dropdown_items);
...
pub fn handle_access_denied(alias: &str, ctx: &PublicRequestContext<'_>) -> Result<HttpResponse> {
    ...
    if user.is_none() {
        ...
        nop_rt_templates::error::serve_404(
