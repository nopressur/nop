...
#### 1. **Markdown Parsing** (`src/markdown/parser.rs`)
...
#### In `src/markdown/parser.rs`:

```rust
use crate::shortcode::{process_text_with_shortcodes, replace_shortcode_placeholders};
...
pub fn handle_my_shortcode(
    ...
    Ok(render_minijinja_template(template_engine, "public/shortcode/my_shortcode.html", context)
        .unwrap_or_else(|_| {
...
Add to `nop/crates/nop-rt-templates/src/engine.rs`:

```rust
"public/shortcode/my_shortcode.html" => {
    Some(include_str!(
        "../../nop-public/src/shortcode/templates/my_shortcode.html"
    ))
}
```
