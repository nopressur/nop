...
use nop_roles::ADMIN_ROLE;
use nop_rt_iam::AuthRequest;
use nop_rt_page_cache::PageMetaCache;
use nop_rt_search_service::{QueryPublicRequest, SearchService};
...
pub(super) async fn search_public(
    ...
    let roles = user
        ...
        .unwrap_or_default();
    let dev_mode_bypass_allowed =
        nop_rt_security::is_dev_mode_bypass_allowed(&req, config.get_ref());
    match search_service.query_public(QueryPublicRequest {
        ...
        Ok(hits) => {
            ...
            let user_roles = user_ref.map(|user| user.roles.as_slice());
            ...
            for hit in hits {
                ...
                // Search prefilters by indexed roles, but PageMetaCache remains the
                // authoritative request-time access check before a hit is returned.
                let (found, allowed) =
                    check_file_access(&page_cache, &lookup, user_roles, dev_mode_bypass_allowed);
                if found && allowed {
                    filtered.push(hit);
                }
            }
